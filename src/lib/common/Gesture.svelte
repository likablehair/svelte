<script lang="ts">
  ...
  export let 
    ...
    diagonalThreshold: number = 100;

  let lastTouch, startTouch, endTouch, startTime: Date, endTime: Date
  ...
  function handleTouchEnd(event) {
    ...
    if(timeDifference < timeThreshold) {
      ...
      if(Math.abs(Math.abs(xDifference) - Math.abs(yDifference)) < diagonalThreshold) return
